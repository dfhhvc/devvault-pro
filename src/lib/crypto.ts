 ...
 *
 * FIX: Uses TextEncoder to convert input to UTF-8 bytes before processing.
 * The previous implementation used string.charCodeAt() which returns UTF-16
 * code units, producing incorrect hashes for non-ASCII characters (Chinese,
 * Emoji, etc.).
 */
export function md5(message: string): string {
  ...
  /**
   * Convert input to a word array.
   * FIX: Uses UTF-8 bytes (via TextEncoder) instead of UTF-16 code units
   * (via charCodeAt) so that non-ASCII characters hash correctly.
   */
  const convertToWordArray = (bytes: Uint8Array) => {
    ...
    while (lByteCount < lMessageLength) {
      ...
      lWordArray[lWordCount] = (lWordArray[lWordCount] || 0) | (bytes[lByteCount] << lBytePosition);
      lByteCount++;
    }
    const lWordCount = (lByteCount - (lByteCount % 4)) / 4;
    lBytePosition = (lByteCount % 4) * 8;
  ...
  const wordToHex = (lValue: number) => {
    ...
    for (let lCount = 0; lCount <= 3; lCount++) {
      ...
      wordToHexValue +=
        "0123456789abcdef".charAt((lByte >>> 4) & 15) +
        "0123456789abcdef".charAt(lByte & 15);
    }
    return wordToHexValue;
  };

  // FIX: Convert to UTF-8 bytes before processing
  const utf8Bytes = new TextEncoder().encode(message);
  const x = convertToWordArray(utf8Bytes);
  let a = 0x67452301;
  let b = 0xefcdab89;
  let c = 0x98badcfe;
  let d = 0x10325476;
...
/**
 * Generates a single unbiased random character from the charset.
